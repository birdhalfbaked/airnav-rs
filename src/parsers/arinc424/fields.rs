...
pub type VfrCheckpointFlagRaw<const START: usize> = FieldRaw<DTYPE_ALPHANUMERIC, START, 1>;
/// 5.159 – ATC Assigned Only
pub type AtcAssignedOnlyRaw<const START: usize> = FieldRaw<DTYPE_ALPHANUMERIC, START, 1>;
/// 5.160 – Units of Altitude for airway restriction (UNIT IND); distinct from §5.133 vertical limit unit
...
/// 5.167 – MLS Back Azimuth Bearing (MLS BAZ BRG)
pub type MlsBackAzimuthBearingRaw<const START: usize> = FieldRaw<DTYPE_NUMERIC, START, 4>;
...
pub type PadDimensionsRaw<const START: usize> = FieldRaw<DTYPE_ALPHANUMERIC, START, 8>;
...
/// 5.194 – Initial Fix / Terminus Fix (Preferred Route)
pub type PreferredRouteInitialFixRaw<const START: usize> = FieldRaw<DTYPE_ALPHANUMERIC, START, 5>;
/// 5.194 – Terminus Fix (Preferred Route)
pub type PreferredRouteTerminusFixRaw<const START: usize> = FieldRaw<DTYPE_ALPHANUMERIC, START, 5>;
...
pub type HoldingLegInboundOutboundIndicatorRaw<const START: usize> =
    FieldRaw<DTYPE_ALPHA, START, 1>;
/// 5.299 – Procedure Referenced Fix Identifier (5 characters max)
